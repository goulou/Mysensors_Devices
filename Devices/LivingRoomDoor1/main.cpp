...
#define NUMBER_OF_INPUT_SENSORS 1 // Total number of attached motion sensors
const uint8_t input_pins[NUMBER_OF_INPUT_SENSORS] PROGMEM = {PD3};
const uint8_t input_ids [NUMBER_OF_INPUT_SENSORS] PROGMEM = {64};
...
//	count ++;
	...
	Serial.println("Sl");
	wdt_disable();
	while(sleep(digitalPinToInterrupt(input_pins[0]), digitalRead(input_pins[0])==LOW?RISING:FALLING, SLEEP_TIME) == true)
	{
		wdt_reset();
		wdt_enable(WDTO_8S);
		wdt_reset();
